...
from simulator.actions.decide_to_talk import DecideToTalk
from simulator.actions.gen_action_details import GenActionDetails
from simulator.actions.gen_daily_schedule import GenDailySchedule
from simulator.actions.gen_hourly_schedule import GenHourlySchedule
from simulator.actions.new_decomp_schedule import NewDecompSchedule
from simulator.actions.summarize_conv import SummarizeConv
from simulator.actions.task_decomp import TaskDecomp
from simulator.actions.wake_up import WakeUp
from simulator.memory.retrieve import new_agent_retrieve
from simulator.plan.converse import agent_conversation
from simulator.utils.utils import get_embedding
...
async def revise_identity(role: "STRole"):
    ...
    plan_note = await LLM().aask(plan_prompt)
    ...
    thought_note = await LLM().aask(thought_prompt)
    ...
    new_currently = await LLM().aask(currently_prompt)
    ...
    new_daily_req = await LLM().aask(daily_req_prompt)
