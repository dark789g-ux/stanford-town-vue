...
from core.const import MARKDOWN_TITLE_PREFIX, MESSAGE_ROUTE_TO_ALL
from core.logs import logger
from core.utils.exceptions import handle_exception
from core.utils.json_to_markdown import json_to_markdown
...
async def get_mime_type(filename: str | Path, force_read: bool = False) -> str:
    ...
    # Upstream MetaGPT shelled out to the Unix ``file --mime-type`` command
    # here. stanford-town-vue is self-contained (and runs on Windows), so we
    # drop that dependency and fall back to the stdlib guess.
    return guess_mime_type or "unknown"
